...
[**service_broker_auth_refresh_token**](AuthenticationApi.md#service_broker_auth_refresh_token) | **GET** /auth/v1/refresh-token | Get a refresh token from a valid access token
...
# **service_broker_auth_refresh_token**
> RefreshTokenResponse service_broker_auth_refresh_token()

Get a refresh token from a valid access token
...
try:
    # Get a refresh token from a valid access token
    api_response = api_instance.service_broker_auth_refresh_token()
    pprint(api_response)
except ApiException as e:
    print("Exception when calling AuthenticationApi->service_broker_auth_refresh_token: %s\n" % e)
```
