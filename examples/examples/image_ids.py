...
configuration.host = os.environ.get('ip4g_endpoint',
    "https://service-broker-api.staging.gpcloudtest.com")
...
if __name__ == '__main__':
    """Display a list of image names and IDs."""
    client = api_client(configuration)
    api = ip4g.PCloudImagesApi(client)
    try:
        api_response = \
            api.pcloud_cloudinstances_images_getall(cloud_instance_id)
        for image in api_response.images:
            print(f"{image.name:<20}{image.image_id}")
    except ApiException as e:
        print("Exception: %s\n" % e)
